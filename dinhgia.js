...
let mack = (ams.get("code") || inputF.value).toUpperCase();
inputF.value = mack;
let jsonData = {};
let lastPrice = 0;
let daysLimit = 90;
...
function loading(i = !0) {
  ...
  const bieudo = document.getElementById("bieudo");
  document.querySelector(".optionbox").style.display = !i ? "block" : "none";
  if (!i && bieudo && (bieudo.innerHTML.trim() === "")) {
	document.querySelector(".optionbox").style.display = "none";
	drawchart(jsonData, typePE, mack, "bieudo");
  }
...
function vechart(data, symbol) {
    ...
    events: {
        load: function () {
            const chart = this;
            chart.container.addEventListener("contextmenu", function (e) {
                e.preventDefault();

                // Lấy point tại vị trí chuột
                const point = chart.hoverPoint;

                if (point) {
                    const fileLink = data[point.x].file;

                    navigator.clipboard.writeText(fileLink)
                        .then(() => {
                            console.log("Đã copy link!", fileLink);
                        })
                        .catch(err => {
        ...
        tooltip: {
            ...
            formatter: function() {
                ...
                return `<b>${point.key}</b><br>` + `${point.series.name}: ${point.y.toFixed(0)}<br>` + `Upside hiện tại: ${data[point.x].upside.toFixed(2)} (%)<br/>` + `LNDP: ${data[point.x].profit ||0} (tỷ)<br/>` + `Nguồn: ${data[point.x].source} `;
            }
        },
        yAxis: {
            title: {
        ...
        series: [{
            ...
            states: {
                hover: {
                    color: isDarkTheme? 'rgba(255,255,255,0.3)': 'green'
                }
            }
        }],
        plotOptions: {
...
function drawchart(data, type, symbol, idx="container") {
    ...
    Highcharts.chart(idx, {
        ...
        xAxis: {
            ...
            crosshair: {
                ...
                color: isDarkTheme?'#666666':'violet', // Màu của đường kẻ dọc (xám nhạt, bạn có thể đổi)
        ...
        tooltip: {
            ...
            valueDecimals: 0, // Hiển thị 0 chữ số thập phân cho giá trị
            formatter: function() {
        ...
        series: [{
            ...
            name: symbol,
            data: vnindexData,
            color: isDarkTheme?'#d1d4dc':'blue' // Màu đen cho VNINDEX
        }, {
            type: 'arearange',
            name: 'Vùng đắt',
            data: PlusRangeData,
...
document.addEventListener("DOMContentLoaded", () => {
    const selectEl = document.getElementById("opt");
    const selectEl2 = document.getElementById("opt2");
    if (selectEl) {
    ...
    if (selectEl2) {
        // Ở đây phải là selectEl2, không phải selectEl
        selectEl2.addEventListener("change", (event) => {
            OptionChange2(event.target.value);
        });
    }
});
